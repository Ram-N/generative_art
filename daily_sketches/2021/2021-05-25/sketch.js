...
function plus3Minus1(seg, nA, nB, bridge, grid, phrase) {
    nA.active = 1
    nA.color = seg.color
    nA.start.active = 1
    nA.end.active = 1
    ...
    if (random() < 0.5) {
        if ((seg.n00) && (seg.n10)) {
            ...
            if ((!seg.n00.start.active) && (!seg.n10.start.active)) {
                ...
                if (bridge && (!bridge.active)) {
                    plus3Minus1(seg, seg.n00, seg.n10, bridge, grid, 'contract');
                    return 1
                }
            }
        }
        ...
        if ((seg.n01) && (seg.n11)) { // down
            if ((!seg.n01.end.active) && (!seg.n11.end.active)) {
                ...
                if (bridge && (!bridge.active)) {
                    plus3Minus1(seg, seg.n01, seg.n11, bridge, grid, 'expand');
                    return 1
                }
            }
        }
...
function initializeSegments(grid) {
    ...
    activateSegmentRect(0, 0, grid.cols, grid.rows, random(palette2))

    //inner rect
    activateSegmentRect(3, 3, 13, 13, colr = random(palette))
    activateSegmentRect(18, 3, 28, 13, colr = random(palette))
    activateSegmentRect(3, 18, 13, 28, colr = random(palette))
    activateSegmentRect(18, 18, 28, 28, colr = random(palette))
