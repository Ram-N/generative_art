...
function createTileGrid(n, cnv) {
    ...
    for (tx = 0; tx < n; tx++) {
        for (ty = 0; ty < n; ty++) {
            x = (1 / (2 * n) + tx / n) * cnv.width + cnv.xMargin
            y = (1 / (2 * n) + ty / n) * cnv.height + cnv.yMargin
            tile = createVector(x, y)
            tile.height = cnv.height / n
            tile.width = cnv.width / n
            tile.nwx = tile.x - tile.width / 2
            tile.nwy = tile.y - tile.height / 2
            tiles.push(tile)
        }
    }
    return tiles
