// Daily Sketch for 2021-06-08
// Ram Narasimhan.

/*
25 squares rotated, slashed
*/
...
function setup() {
    ...
    for (col = 0; col < nCols; col++) {
        for (row = 0; row < nRows; row++) {
            ...
            side = xStep * mult;
            push();
            translate(col * xStep + x2 + cnv.xMargin,
                row * yStep + y2 + cnv.yMargin);
            push();
            rotate(PI / 4 * id)
            rect(0, 0, side, side);
            pop();
            ...
            line(-x2 * 0.85, -y2 * random(-1, 1), x2 * 0.85, y2 * random(-1, 1))
            noStroke();
            fill(200);
            circle(0, 0, 20);
            pop();
