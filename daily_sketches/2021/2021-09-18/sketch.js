...
function nestedMandala(x, y) {
    ...
    for (let mand = 0; mand < numMand; mand++) {
        ...
        for (let cop = 0; cop < numCopies; cop++) {
            push();
            strokeWeight(2)
            stroke(random(palette2));
            translate(x, y);
            rotate(90 / numCopies * cop);
            renderSpiro(r1, r2, p);
            pop();
        }
    }
...
// Spirograph equations: https://linuxgazette.net/133/luana.html
//    x(t,R,r,p) = (R-r)*cos(t) + p*cos((R-r)*t/r)
//   y(t,R,r,p) = (R-r)*sin(t) - p*sin((R-r)*t/r)
function renderSpiro(r1, r2, p) {
    let px = 0;
    let py = 0;
