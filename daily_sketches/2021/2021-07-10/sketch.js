// Daily Sketch for 2021-07-10
// Ram Narasimhan.

/*
Multiple Mandalas, within other mandalas, two levels
...
function draw() {
    ...
    for (r of [0, 1]) {
        for (mandY = 0; mandY < clockStops; mandY++) {
            push();
            x = spoke[r] * cos(slice * mandY + halfslice * r)
            y = spoke[r] * sin(slice * mandY + halfslice * r)
            translate(width / 2 + x, height / 2 + y)
            prepMandalaSpecs()
            renderMandala(radius[r], incr);
            pop();
        }
    }
