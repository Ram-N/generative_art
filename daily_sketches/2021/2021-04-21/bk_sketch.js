...
function draw() {
    ...
    for (let iter = 0; iter < generationSpeed && j < 20; iter++) {
        if (!occupied[i][j]) {
            ...
            for (let {
                ...
                print('neighbor', nColumn, neighRow)
                if (!occupied[nColumn][neighRow]) {
                    fill(fillColor)
                    drawTriangle(i, j)
                    drawTriangle(nColumn, neighRow)
                    print(i, j, nColumn, neighRow, 'have the same color...')
                    occupied[i][j] = true;
                    occupied[nColumn][neighRow] = true;
                    break
                }
            }
        }
