...
function setup() {
    ...
    for (x = cnv.xMargin; x < cnv.xMargin + cnv.width + 10; x += 26) {
        for (y = cnv.yMargin; y < cnv.width + cnv.yMargin + 10; y += 10) {
            fxy = complex_valued(x, y)
            cmap = color_map(fxy, x, y)
            stroke(palette[cmap])
            fill(palette[cmap])
            rect(x, y, 5 + random(10, 25), 5 + random(5))
        }
    }
