...
function setup() {
    ...
    palette = ['red', 'black', 'gray', 'orange', 'brown']
...
function get_fxy_min_max() {
    currMax = -1e10
    currMin = 1e10
    xStep = int(cnv.width / 50)
    yStep = int(cnv.height / 50)
...
function complex_valued(x, y, verbose = false) {
    comp1 = y * cos(x) * abs(sin(x))
    comp2 = (x * y) / 100
    comp3 = - abs(0.02 * x) + 4 * y
