class Ripple {
    ...
    createPath(grid) {
        ...
        let max_loop = max(grid.rows, grid.cols) * 10; //circuit breaker
        let iter = 0;
    ...
    terminatePath(GPt, grid) {
        ...
        if ((GPt != this.source) && (GPt.row == grid.rows)) {
            return 1
        }
        if ((GPt != this.source) && (GPt.row == 0)) {
            return 1
        }
...
function dist_metric(nx, ny, tx, ty) {
    ...
    distance = Math.abs(nx - tx) + Math.abs(ny - ty)
