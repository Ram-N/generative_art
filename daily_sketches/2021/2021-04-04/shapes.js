...
function renderRectEcho(rct, grid) {
  push();
  strokeWeight(8)
  let shade = 0;
  for (pe = 0; pe < rct.perimeter.length - 1; pe++) {
    stroke(255 - shade, 0, 0)
    p = rct.perimeter[pe];
    q = rct.perimeter[pe + 1];
    //point(p.x, p.y)

    if (p.col == q.col) {//vertical
      pLeft = grid.getGlobalCoords(p.col - 1, p.row)
      pRight = grid.getGlobalCoords(p.col + 1, p.row)
