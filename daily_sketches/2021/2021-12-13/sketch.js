...
function setup() {
    ...
    for (it = 0; it < numSeeds; it++) {
        ...
        while (currSeg) {
            count += 1
            newSeg = findAFreeNeighbor(currSeg);
            if (newSeg) {
                print(newSeg.id)
                ...
                print('no FREE neigh for', currSeg)
            }
            currSeg = newSeg
        }
    }
...
function findAFreeNeighbor(currSeg) {
    if (currSeg.neighbors) {
        for (attempt = 0; attempt < 10; attempt++) {
            nrand = random([0, 1, 2, 3, 4, 5])
            neigh = currSeg.neighbors[nrand]
            if (validNeighbor(currSeg, neigh)) {
                //print('orient', currSeg.orientation, nrand)
...
function validNeighbor(currSeg, neigh) {
    if (neigh) {
