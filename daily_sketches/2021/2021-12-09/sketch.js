...
function setup() {
    ...
    for (it = 0; it < 200; it++) {
        ...
        sw = 10
        grid.segments[rn].display(colr, sw = sw)
        if (grid.segments[rn].neighbors) {
            nrand = random([0, 1, 2, 3])
            if (grid.segments[rn].neighbors[nrand]) {
                grid.segments[rn].neighbors[nrand].display(colr, sw = sw)
            }
        }
    }
