...
function drawMetro(pal) {
    ...
    crowdThreshold = 4; //not being used currently

    pt = new Point(int(width / 2), int(height / 20));
    pt.dir = 0; //directions go from 0..7 --> 8 Cardinals
    fill(255)
    for (cl = 0; cl < numClusters; cl++) {
        print(pt.x, pt.y, cl)
        terminus(pt);
        pt = makeCluster('Short', pt, clusterSize, minDist = 5, maxDist = 10, pal)
        terminus(pt)
        //now, make the long jump
        pt = makeCluster('LongDistance', pt, 1, minDist = 40, maxDist = 60, purples)
    }
    terminus(pt);
}


function terminus(pt) {
    push();
    fill(255)
    noStroke();
    circle(pt.x, pt.y, 40)
...
function makeCluster(sym, pt, clusterSize, minDist, maxDist, pal) {
    ...
    for (i = 0; i < clusterSize; i++) {
        ...
        pt = move45(pt, _dir, minDist, maxDist); //move and update
    }
    return (pt)
}
...
/*
take a point
move to a new position (45 degrees) in discrete jumps
draw the line
*/
function move45(pt, newDir, minDist, maxDist) {
