...
class IsoTriangle {
    ...
    displayNeighbors(grid) {
        ...
        let sideOffset = 0;
        let rowOffset = 0;
        let flip = this.orientation == 1 ? 0 : 1

        print(this, 'base', this.row, this.row % 2)
...
class HexGrid {
    ...
    */
    createIsoTriangles() {
        ...
        for (let co of this.points) {
            for (let g of co) {
                tri = new IsoTriangle(g.col, g.row, this.triangleHt, this, 0)
                triangles.push(tri)
                tri = new IsoTriangle(g.col, g.row, this.triangleHt, this, 1)
                triangles.push(tri)
            }
        }
        return triangles
