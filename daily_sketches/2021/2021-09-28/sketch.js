...
function setup() {
    ...
    let sh = new Shingle(nw, ne, se, sw, random(palList));
    shingleList.push(sh);
    ...
    subdivide(sh0, 0, shingleList);

    for (s = 0; s <= numSubdivisions; s += 1) {
        //pick a shingle to divide
        sh0 = random(shingleList);
        subdivide(sh0, 1, shingleList);
    }
...
*/
function subdivide(sh0, maintainColor, shingleList) {
    ...
    if ((sh0.open) && (sh0.vsize > vSizeCutoff) && (sh0.hsize > hSizeCutoff)) { // untouched, so go ahead and subdivide
        ...
        let sh = new Shingle(nw, ne, se, sw, pal);
        shingleList.push(sh);
        ...
        sh = new Shingle(nw, ne, se, sw, pal);
        shingleList.push(sh);
