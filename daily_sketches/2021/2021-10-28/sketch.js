...
function setup() {
    ...
    for (t of tg.tiles) {
        ...
        selected = Hred_orange[t.col % 5];
        strokeWeight(3);
        populateTile(t.cx + jx, t.cy + jy, tg.width, toss, selected)
        selected = HrainbowDash[t.row % 5];
        strokeWeight(3);
        populateTile(t.cx + jx, t.cy + jy, tg.width, toss, selected)
        pop();
...
function populateTile(x, y, stepSize, toss = 0, colr) {

    noStroke();
    push();
    jx = jitter(0);
    jy = jitter(0);
    makeTile(toss, x + jx, y + jy, stepSize, colr);

}


patternDict = { // each triplet is a junction showing its position wrt cx,cy=0,0
    6: [[-2, 1, 1], [-1, 2, 0]],
    24: [[2, 1, 1], [1, 2, 0]],
    96: [[2, -1, 1], [1, -2, 0]],
    129: [[-2, -1, 1], [-1, -2, 0]],
...
/* Renders a composite tile based on its base elements
   These in turn are made up of connections steppedLines etc*/
function tilePattern(patternNum, x, y, qlength) {

    jnList = [];
    //print(patternNum, patternDict[patternNum], x, y)
    for (elem of compositeTile[patternNum]) {
        for (jn of patternDict[elem]) { // Junction come in pairs
            jnList.push(jn)
        }
    }
    //we now have a long list of jn triplets to be rendered...
    //Logic: generate a random point inside the tile.
    // connect 2 random junctions to the point and delete the junctions.
    ...
    jLen = jnList.length
    for (p = 0; p < jLen / 2; p++) {
        ptSel = random(fourCorners)
        ptx = ptSel[0] * qlength;
        pty = ptSel[1] * qlength;

        idx = int(random(jnList.length))//Pick a jn

        jn = jnList[idx]
        steppedLine(jn[0] * qlength, jn[1] * qlength, ptx, pty, hStart = jn[2]);
        jnList.splice(idx, 1);//remove item at position idx

        idx = int(random(jnList.length))
        jn = jnList[idx]
        steppedLine(jn[0] * qlength, jn[1] * qlength, ptx, pty, hStart = jn[2]);
        jnList.splice(idx, 1);//remove item at position idx
...
function makeTile(tnum, x, y, stepSize, colr) {
