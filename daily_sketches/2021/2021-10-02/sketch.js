...
/*
Keywords: fractals, in-a-box, shapes, subdivision
*/
...
function setup() {
    ...
    for (p of pgrid.panels) {
        ...
        divideType = random(['Triangle', 'Quad', 'Mixed'])
        divideType = 'Triangle'
        boxDivision(box, divideType)
    }
...
function boxDivision(box, divideType = 'Mixed') {
    let shingleList = [];
    sh = initializeShingle(box);
    shingleList.push(sh);

    if ((divideType == 'Quad') || (divideType == 'Mixed')) {
        starting_cutoff = box.w * box.h * 0.2
    ...
    if ((divideType == 'Triangle') || (divideType == 'Mixed')) {
        for (s = 0; s <= numTdivisions; s += 1) {
            sh0 = random(shingleList); //pick a shingle to divide
            mColor = random([0, 1, 1, 1, 1, 1, 1])
...
function initializeShingle(box) {
    //box has x y w h
    let nw = new Point(box.x, box.y)
    let ne = new Point(box.x + box.w, box.y)
    let sw = new Point(box.x, box.y + box.h)
    let se = new Point(box.x + box.w, box.y + box.h)
...
function tridivide(sh0, maintainColor, shingleList) {
    ...
    minRatio = 0.5 // a line AB gets cuts into two some distance between A and B. This is the min ratio
    maxRatio = 0.5 // a line AB gets cuts into two some distance between A and B. This is the max ratio
    ...
    if (sh0.type == 't') {
        if ((sh0.open) && (sh0.ab > vSizeCutoff) && (sh0.bc > hSizeCutoff) && (sh0.ac > hSizeCutoff)) { // untouched, so go ahead and divide
            ...
            let midAC = getInbetweenPtOnLine(sh0.ptA.x, sh0.ptA.y, sh0.ptC.x, sh0.ptC.y, random(minRatio, maxRatio))
            let midAB = getInbetweenPtOnLine(sh0.ptA.x, sh0.ptA.y, sh0.ptB.x, sh0.ptB.y, random(minRatio, maxRatio))
            let midBC = getInbetweenPtOnLine(sh0.ptB.x, sh0.ptB.y, sh0.ptC.x, sh0.ptC.y, random(minRatio, maxRatio))
            ...
            let coin = random();
            if (coin < 0.33) { //Divide at AC = t1 {A, B, midAC} {midAC, B, C}
                sh1 = new Triangle('t', sh0.ptA, sh0.ptB, midAC, pal1); //make AB-midAC
                sh2 = new Triangle('t', midAC, sh0.ptB, sh0.ptC, pal2); //make midAC-BC
            } else if (coin < 0.66) { //divide at BC  - t1 ={A, B, midBC} {midBC, C, A}
                sh1 = new Triangle('t', sh0.ptA, sh0.ptB, midBC, pal1);
                sh2 = new Triangle('t', midBC, sh0.ptC, sh0.ptA, pal2);
            } else { //divide at AB t1 {C, A, midAB} {C, midAb, B}
                sh1 = new Triangle('t', sh0.ptC, sh0.ptA, midAB, pal1);
                sh2 = new Triangle('t', sh0.ptC, midAB, sh0.ptB, pal2);
            }
