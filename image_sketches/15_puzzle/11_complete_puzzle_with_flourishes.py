...
This is the final script in the Tutorial.
We have the game working fine. But let's add a few flourishes to it.

Specifically, we will do three things:
    1. Give a visual cue when the tile comes to it orignal position. No tint.
    2. If all the tiles are in their correct positions, blink! or have a white-out!
    3. Keep track of total moves made
...
def render_slices_by_given_sequence(imglist, sequence):
    ...
    for px in range(4):
        for py in range(4):
            ...
            # Add a flourish: If the image tile is misplaced, tint it. else don't.
            if sequence[snum] != snum:  # it is not in the right position
                tint(255, 127, 0)
            else:
                tint(255, 255, 255)  # white is the same as no tint...

            image(imglist[sequence[snum]], smx * px, py * smy)


def all_done(sequence):
    """ check if every image is ints right place."""
...
def draw():
    global smx, smy, w_compress, h_compress, imglist, order, hole, total_moves
    ...
    if all_done(order):
        print("Completed in ", total_moves, "moves")
        noLoop()
